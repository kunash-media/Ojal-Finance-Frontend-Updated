...
const FixedDeposit = () => {
  ...
  const fetchFdAccountsData = useCallback(async () => {
    ...
    try {
      ...
      const accountCheckPromises = branchUsers.map(async (user) => {
        try {
          const response = await fetch(`https://api.ojalmsfoundation.in/api/fds/get-all-fds-by-userId/${user.userId}`);
          if (response.ok) {
  ...
  const handleConfirm = async () => {
    try {
      const apiUrl = `https://api.ojalmsfoundation.in/api/accounts/${selectedUser.userId}/fd`;
      const payload = {
  ...
  const handleUpdateConfirm = async () => {
    try {
      const apiUrl = `https://api.ojalmsfoundation.in/api/fds/patch-fd-by-accNum/${currentFdAccountStatus}`;
      const payload = {
        principalAmount: parseFloat(fdFormData.principalAmount),
        interestRate: parseFloat(fdFormData.interestRate),
        tenureMonths: parseInt(fdFormData.tenureMonths)
  ...
  const handleDeleteAccount = async () => {
    try {
      const apiUrl = `https://api.ojalmsfoundation.in/api/fds/delete-fd-by-accNum/${accountToDelete}`;
      const response = await fetch(apiUrl, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
  ...
  const handleDeleteAllFDs = async () => {
    try {
      const apiUrl = `https://api.ojalmsfoundation.in/api/fds/delete-all-fds-by-userId/${selectedUser.userId}`;
      const response = await fetch(apiUrl, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
